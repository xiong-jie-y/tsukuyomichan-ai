...
class TsukuyomichanVisualizationGenerator:
    def __init__(self, clock, upscale=True):
    ...
    def generate(self):
        ...
        background = PIL.Image.new('RGBA', pil_image.size, (0,255,0))
        ...
        if self.upscale:
            ...
            s = time.time()
            # import IPython; IPython.embed()
            # output_image_2 = self.super_resolution_session.run([], {"input.1": [np.array(pil_image).reshape(3, 256, 256) / 255]})[0][0]
            # pil_image2 = PIL.Image.fromarray(numpy.uint8(numpy.rint(output_image_2.reshape(3, 1024*1024).transpose().reshape(1024, 1024, 3) * 255)), mode='RGB')
...
class TsukuyomichanVisualizer:
    ...
    def __init__(self, talksoft, clock=WallClock(), wav_output=Speaker()):
        d = ModelDownloader()
        ...
        self.visualization_generator = TsukuyomichanVisualizationGenerator(self.clock)
